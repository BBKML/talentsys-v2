<x-app-layout title="Niveaux — Académique">
@push('styles')
<style>
...
@php
$data = $niveaux->map(fn($n) => [
    'id'=>$n->id,'libelle'=>$n->libelle,'code'=>$n->code??'',
    'type_niveau'=>$n->type_niveau??'','credits_requis'=>$n->credits_requis??'',
    'ordre'=>$n->ordre??0,'actif'=>$n->id_statut==1,
]);
$types = ['Licence','Master','Doctorat','BTS','DUT','Classe Prépa','Autre'];
@endphp
...
<div x-data="page({{ $data }})" class="space-y-5">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold" style="color:#1E293B">Niveaux</h1>
            <p class="text-sm mt-0.5" style="color:#94A3B8"><span x-text="items.length"></span> niveau(x)</p>
        </div>
        <button @click="openCreate()" class="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl text-white text-sm font-semibold shadow-sm hover:opacity-90" style="background:var(--primary)">
            <i class="ri-add-line"></i> Nouveau niveau
        </button>
    </div>
    ...
    <div class="grid grid-cols-3 gap-4">
        <div class="bg-white rounded-2xl p-4 flex items-center gap-4 shadow-sm border border-gray-100">
            <div class="w-11 h-11 rounded-xl flex items-center justify-center" style="background:rgba(90,103,216,.12)"><i class="ri-stack-fill text-xl" style="color:var(--primary)"></i></div>
        ...
        <div class="bg-white rounded-2xl p-4 flex items-center gap-4 shadow-sm border border-gray-100">
            ...
            <div><p class="text-xs font-semibold" style="color:#94A3B8">Actifs</p><p class="text-2xl font-bold" style="color:#1E293B" x-text="items.filter(i=>i.actif).length"></p></div>
        </div>
        <div class="bg-white rounded-2xl p-4 flex items-center gap-4 shadow-sm border border-gray-100">
            ...
            <div><p class="text-xs font-semibold" style="color:#94A3B8">Inactifs</p><p class="text-2xl font-bold" style="color:#1E293B" x-text="items.filter(i=>!i.actif).length"></p></div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        <div class="flex items-center gap-3 px-5 py-4 border-b border-gray-100">
            <div class="relative flex-1" style="max-width:280px">
                ...
                <input x-model="search" @input="currentPage=1" type="text" placeholder="Rechercher un niveau..." class="f-input" style="padding:8px 12px 8px 34px">
            </div>
        ...
        <table class="w-full">
            <thead><tr style="border-bottom:1px solid #F1F5F9;background:#FAFBFC">
                <th class="tbl-th" style="width:90px">Code</th>
                <th class="tbl-th" style="width:30%">Libellé</th>
                <th class="tbl-th" style="width:20%">Type</th>
                <th class="tbl-th text-center" style="width:110px">Crédits req.</th>
                <th class="tbl-th text-center" style="width:80px">Ordre</th>
            ...
            <tbody>
                <template x-if="paginated.length===0">
                    <tr><td colspan="7" class="py-20 text-center">
                        <div class="w-14 h-14 rounded-2xl mx-auto mb-3 flex items-center justify-center" style="background:#F1F5F9"><i class="ri-stack-line text-3xl" style="color:#CBD5E1"></i></div>
                        <p class="text-sm font-semibold" style="color:#64748B">Aucun niveau enregistré</p>
                    </td></tr>
                </template>
                <template x-for="r in paginated" :key="r.id">
                    <tr style="border-bottom:1px solid #F8FAFC" class="hover:bg-slate-50 transition-colors">
                        <td class="tbl-td">
                            <span class="inline-flex items-center px-2.5 py-1 rounded-lg text-[11px] font-bold font-mono" style="background:rgba(90,103,216,.1);color:var(--primary)" x-text="r.code||'—'"></span>
                        </td>
                        <td class="tbl-td font-semibold" style="color:#1E293B" x-text="r.libelle"></td>
                        <td class="tbl-td text-[12px]" style="color:#64748B" x-text="r.type_niveau||'—'"></td>
                        <td class="tbl-td text-center text-[12px]" style="color:#64748B" x-text="r.credits_requis||'—'"></td>
                        <td class="tbl-td text-center">
                            <span class="text-xs font-bold font-mono px-2 py-0.5 rounded-md" style="background:#F1F5F9;color:#64748B" x-text="r.ordre||0"></span>
                        </td>
                        <td class="tbl-td text-center">
                            <form :action="'/niveaux/'+r.id+'/statut'" method="POST" style="display:inline">
                                @csrf @method('PATCH')
                        ...
                        <td class="tbl-td">
                            <div class="flex items-center justify-end gap-1">
                                ...
                                <form :action="'/niveaux/'+r.id" method="POST" style="display:inline" @submit.prevent="if(confirm('Supprimer ce niveau ?')) $el.submit()">
                                    @csrf @method('DELETE')
    ...
    <template x-if="modal">
        <div class="fixed inset-0 z-50 flex items-center justify-center p-4" style="background:rgba(15,23,42,.5)">
            <div class="bg-white rounded-2xl shadow-2xl w-full" style="max-width:500px" @click.stop>
                <div class="flex items-center gap-3 px-6 py-4">
                    <div class="w-10 h-10 rounded-xl flex items-center justify-center" style="background:rgba(90,103,216,.12)"><i class="ri-stack-fill text-xl" style="color:var(--primary)"></i></div>
                    <div class="flex-1"><h2 class="text-[15px] font-bold" style="color:#1E293B" x-text="editing?'Modifier le niveau':'Nouveau niveau'"></h2></div>
                ...
                <form :action="editing?'/niveaux/'+form.id:'{{ route('niveaux.store') }}'" method="POST" @submit="submitting=true">
                    @csrf
                    ...
                    <div class="px-6 pt-5 pb-2 space-y-4">
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="text" name="libelle" :value="form.libelle" required class="f-input" placeholder="Ex : Licence 1">
                            </div>
                            <div>
                                <label class="f-label">Code <span style="color:#EF4444">*</span></label>
                                <input type="text" name="code" :value="form.code" required class="f-input" placeholder="Ex : L1">
                            </div>
                        </div>
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                <label class="f-label">Type de niveau</label>
                                <div x-data="sSelect([{v:'Licence',l:'Licence'},{v:'Master',l:'Master'},{v:'Doctorat',l:'Doctorat'},{v:'BTS',l:'BTS'},{v:'DUT',l:'DUT'},{v:'Classe Prépa',l:'Classe Prépa'},{v:'Autre',l:'Autre'}], form.type_niveau, 'Rechercher un type...')"
                                     class="relative" @click.outside="open=false">
                                    <input type="hidden" name="type_niveau" :value="v">
                                    <div class="relative">
                            ...
                            <div>
                                <label class="f-label">Crédits requis</label>
                                <input type="number" name="credits_requis" :value="form.credits_requis" class="f-input" placeholder="Ex : 60" min="0">
                            </div>
                        </div>
                        <div style="max-width:150px">
                            <label class="f-label">Ordre d'affichage</label>
                            <input type="number" name="ordre" :value="form.ordre" class="f-input" placeholder="1" min="0">
                        </div>
                    </div>
...
function page(data){
    return {
        ...
        form:{id:'',libelle:'',code:'',type_niveau:'',credits_requis:'',ordre:0},
        get filtered(){ const q=this.search.toLowerCase(); return q?this.items.filter(i=>i.libelle.toLowerCase().includes(q)||(i.code||'').toLowerCase().includes(q)):this.items; },
        ...
        openCreate(){ this.editing=false; this.submitting=false; this.form={id:'',libelle:'',code:'',type_niveau:'',credits_requis:'',ordre:0}; this.modal=true; },
