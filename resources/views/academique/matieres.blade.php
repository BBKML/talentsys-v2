<x-app-layout title="Matières — Académique">
@push('styles')
<style>
...
$data = $matieres->map(fn($m) => [
    'id'=>$m->id,'libelle'=>$m->libelle,
    'coefficient'=>$m->coefficient??'','credit'=>$m->credit??'',
    'id_ue'=>$m->id_ue,'ue'=>$m->ue?->libelle??'—',
    'id_filiere'=>$m->id_filiere,'filiere'=>$m->filiere?->libelle??'—',
    'id_niveau'=>$m->id_niveau,'niveau'=>$m->niveau?->libelle??'—',
    'id_decoupage_annee'=>$m->id_decoupage_annee,'decoupage'=>$m->decoupage?->libelle??'—',
    'actif'=>$m->id_statut==1,
]);
$uesJson       = $ues->map(fn($u)=>['id'=>$u->id,'libelle'=>$u->libelle]);
...
<div x-data="page({{ $data }}, {{ $uesJson }}, {{ $filieresJson }}, {{ $niveauxJson }}, {{ $decoupagesJson }})" class="space-y-5">

    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold" style="color:#1E293B">Matières</h1>
            <p class="text-sm mt-0.5" style="color:#94A3B8"><span x-text="items.length"></span> matière(s)</p>
        </div>
    ...
    <div class="grid grid-cols-4 gap-4">
        <div class="bg-white rounded-2xl p-4 flex items-center gap-4 shadow-sm border border-gray-100">
            <div class="w-11 h-11 rounded-xl flex items-center justify-center" style="background:rgba(90,103,216,.12)"><i class="ri-flask-fill text-xl" style="color:var(--primary)"></i></div>
            <div><p class="text-xs font-semibold" style="color:#94A3B8">Total matières</p><p class="text-2xl font-bold" style="color:#1E293B" x-text="items.length"></p></div>
        </div>
        <div class="bg-white rounded-2xl p-4 flex items-center gap-4 shadow-sm border border-gray-100">
        ...
        <div class="bg-white rounded-2xl p-4 flex items-center gap-4 shadow-sm border border-gray-100">
            <div class="w-11 h-11 rounded-xl flex items-center justify-center" style="background:rgba(100,116,139,.1)"><i class="ri-scales-3-fill text-xl" style="color:#64748B"></i></div>
            <div><p class="text-xs font-semibold" style="color:#94A3B8">Moy. coefficient</p><p class="text-2xl font-bold" style="color:#1E293B" x-text="items.length?Math.round(items.reduce((s,i)=>s+(parseFloat(i.coefficient)||0),0)/items.length*10)/10:'0'"></p></div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        <div class="flex items-center gap-3 px-5 py-4 border-b border-gray-100">
            <div class="relative flex-1" style="max-width:280px">
                ...
                <input x-model="search" @input="currentPage=1" type="text" placeholder="Rechercher une matière..." class="f-input" style="padding:8px 12px 8px 34px">
            </div>
        </div>
        <table class="w-full">
            <thead><tr style="border-bottom:1px solid #F1F5F9;background:#FAFBFC">
                <th class="tbl-th" style="width:22%">Libellé</th>
                <th class="tbl-th" style="width:18%">UE</th>
                <th class="tbl-th" style="width:15%">Filière</th>
                <th class="tbl-th" style="width:12%">Niveau</th>
                <th class="tbl-th" style="width:14%">Découpage</th>
                <th class="tbl-th text-center" style="width:70px">Coef.</th>
                <th class="tbl-th text-center" style="width:70px">Crédits</th>
                <th class="tbl-th text-center" style="width:100px">Statut</th>
                <th class="tbl-th text-right" style="width:80px">Actions</th>
            </tr></thead>
            <tbody>
                <template x-if="paginated.length===0">
                    <tr><td colspan="9" class="py-20 text-center">
                        <div class="w-14 h-14 rounded-2xl mx-auto mb-3 flex items-center justify-center" style="background:#F1F5F9"><i class="ri-flask-line text-3xl" style="color:#CBD5E1"></i></div>
                        <p class="text-sm font-semibold" style="color:#64748B">Aucune matière enregistrée</p>
                    </td></tr>
                </template>
                <template x-for="r in paginated" :key="r.id">
                    <tr style="border-bottom:1px solid #F8FAFC" class="hover:bg-slate-50 transition-colors">
                        ...
                        <td class="tbl-td text-[12px]">
                            <span class="text-[11px] font-semibold px-2 py-0.5 rounded-md" style="background:rgba(90,103,216,.1);color:var(--primary)" x-text="r.ue"></span>
                        </td>
                        ...
                        <td class="tbl-td text-[12px]" style="color:#94A3B8" x-text="r.decoupage"></td>
                        <td class="tbl-td text-center text-[12px] font-mono font-bold" style="color:#475569" x-text="r.coefficient||'—'"></td>
                        <td class="tbl-td text-center text-[12px] font-mono" style="color:#94A3B8" x-text="r.credit||'—'"></td>
                        <td class="tbl-td text-center">
                            <form :action="'/matieres/'+r.id+'/statut'" method="POST" style="display:inline">
                                @csrf @method('PATCH')
                        ...
                        <td class="tbl-td">
                            <div class="flex items-center justify-end gap-1">
                                ...
                                <form :action="'/matieres/'+r.id" method="POST" style="display:inline" @submit.prevent="if(confirm('Supprimer cette matière ?')) $el.submit()">
                                    @csrf @method('DELETE')
        ...
        <div class="fixed inset-0 z-50 flex items-center justify-center p-4" style="background:rgba(15,23,42,.5)">
            <div class="bg-white rounded-2xl shadow-2xl w-full" style="max-width:560px" @click.stop>
                <div class="flex items-center gap-3 px-6 py-4">
                    <div class="w-10 h-10 rounded-xl flex items-center justify-center" style="background:rgba(90,103,216,.12)"><i class="ri-flask-fill text-xl" style="color:var(--primary)"></i></div>
                    <div class="flex-1"><h2 class="text-[15px] font-bold" style="color:#1E293B" x-text="editing?'Modifier la matière':'Nouvelle matière'"></h2></div>
                ...
                <form :action="editing?'/matieres/'+form.id:'{{ route('matieres.store') }}'" method="POST" @submit="submitting=true">
                    @csrf
                    <template x-if="editing"><input type="hidden" name="_method" value="PUT"></template>
                    <div class="px-6 pt-5 pb-2 space-y-4">
                        <div>
                            <label class="f-label">Libellé <span style="color:#EF4444">*</span></label>
                            <input type="text" name="libelle" :value="form.libelle" required class="f-input" placeholder="Ex : Algorithmique et Programmation">
                        </div>
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                <label class="f-label">Coefficient</label>
                                <input type="number" name="coefficient" :value="form.coefficient" class="f-input" placeholder="Ex : 2" min="0" step="0.5">
                            </div>
                            <div>
                                <label class="f-label">Crédits</label>
                                <input type="number" name="credit" :value="form.credit" class="f-input" placeholder="Ex : 4" min="0">
                            </div>
                        </div>
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                <label class="f-label">Unité d'Enseignement</label>
                                <div x-data="sSelect(ues.map(u=>({v:u.id,l:u.libelle})), form.id_ue, 'Rechercher une UE...')"
                                     class="relative" @click.outside="open=false">
                                    <input type="hidden" name="id_ue" :value="v">
                                    <div class="relative">
                                        <i class="ri-search-line absolute left-3 top-1/2 -translate-y-1/2 text-sm" style="color:#CBD5E1;pointer-events:none"></i>
                                        <input x-model="s" @focus="open=true" @input="open=true" type="text" class="f-input" style="padding-left:32px" :placeholder="ph" autocomplete="off">
                                    </div>
                            ...
                            <div>
                                <label class="f-label">Découpage / Période</label>
                                <div x-data="sSelect(decoupages.map(d=>({v:d.id,l:d.libelle})), form.id_decoupage_annee, 'Rechercher un découpage...')"
                                     class="relative" @click.outside="open=false">
                                    <input type="hidden" name="id_decoupage_annee" :value="v">
                                    <div class="relative">
                                        <i class="ri-search-line absolute left-3 top-1/2 -translate-y-1/2 text-sm" style="color:#CBD5E1;pointer-events:none"></i>
                                        <input x-model="s" @focus="open=true" @input="open=true" type="text" class="f-input" style="padding-left:32px" :placeholder="ph" autocomplete="off">
                                    </div>
...
@push('scripts')
<script>
function page(data, ues, filieres, niveaux, decoupages){
    return {
        items:data, ues:ues, filieres:filieres, niveaux:niveaux, decoupages:decoupages,
        search:'', perPage:10, currentPage:1, modal:false, editing:false, submitting:false,
        form:{id:'',libelle:'',coefficient:'',credit:'',id_filiere:'',id_niveau:'',id_ue:'',id_decoupage_annee:''},
        get filtered(){ const q=this.search.toLowerCase(); return q?this.items.filter(i=>i.libelle.toLowerCase().includes(q)||i.filiere.toLowerCase().includes(q)||i.ue.toLowerCase().includes(q)):this.items; },
        ...
        openCreate(){ this.editing=false; this.submitting=false; this.form={id:'',libelle:'',coefficient:'',credit:'',id_filiere:'',id_niveau:'',id_ue:'',id_decoupage_annee:''}; this.modal=true; },
